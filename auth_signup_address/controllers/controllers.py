...
class AuthSignupHomeExt(AuthSignupHome):
    ...
    def get_auth_signup_qcontext(self):
        ...
        if get_param('auth_signup_with_birthday') == 'True':
            qcontext['auth_signup_with_birthday'] = True
    ...
    def do_signup(self, qcontext):
        ...
        values = {key: qcontext.get(key) for key in ('login', 'name', 'password', 'phone', 'birthday',
        ...
        if values.get('birthday'):
            values['birthday'] = datetime.strptime(values.get('birthday'), '%Y-%m-%d')
