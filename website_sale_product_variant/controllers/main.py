...
class WebsiteSaleProductVariant(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        # Product = request.env[website.shop_model]
        # attributes
        ...
        if filter_by_price_enabled:
            ...
            Product = request.env[website.shop_model].with_context(bin_size=True)  # MODIFIED #
            ...
            query = f"""
                SELECT COALESCE(MIN({Product._table}.list_price), 0) * {conversion_rate}, COALESCE(MAX({Product._table}.list_price), 0) * {conversion_rate}
                  FROM {from_clause}
                 WHERE {where_clause}
            """
